...
def flush_to_xls(prod_list, store_list):
    ...
    for key in store_list :
        item_arr=[]
        item_arr=prod_list[key]
        wsStore.write(index, 0, item_arr[0])
        wsStore.write(index, 1, item_arr[1])
        wsStore.write(index, 2, item_arr[2])
        wsStore.write(index, 3, item_arr[3])
        index=index+1
