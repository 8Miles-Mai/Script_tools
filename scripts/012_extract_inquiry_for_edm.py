...
def send_edm(inquiry_list):
    ...
    for id in inquiry_list:
        if not id:
            continue
        params = urllib.urlencode({'inquiryId' : id, 'subject' : inquiry_list[id][1]})
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/plain"}
        conn = httplib.HTTPConnection("192.168.24.151", 58080)
        conn.request("POST", "/gmsoa/soaemailservice/sendInquiryMailByInquiryId.gm", params, headers)
