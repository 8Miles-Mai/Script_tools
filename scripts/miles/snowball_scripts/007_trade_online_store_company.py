    ...
    sql="""
        UPDATE `m2cchinaerp`.`store$company` s, ( 
            ...
            WHERE 1=1
                ...
                AND head_banner.`comp_id` = sc.`comp_id` AND head_banner.`cfg_banner_id` = head_banner_cfg.`cfg_banner_id`
                AND head_banner_cfg.`type` = 1 AND head_banner.`status` = 7 AND  head_banner.`image` IS NOT NULL 
                AND hot_banner.`comp_id` = sc.`comp_id` AND hot_banner.`cfg_banner_id` = hot_banner_cfg.`cfg_banner_id`
                AND hot_banner_cfg.`type` = 2 AND hot_banner.`status` = 7 AND  hot_banner.`image` IS NOT NULL AND hot_banner.`link_entity_id` IS NOT NULL
                AND EXISTS (
                    SELECT NULL FROM `gmcore`.`pro_product_base` pb
                    WHERE pb.`status` = 7 AND pb.`comp_id` = sc.`comp_id` GROUP BY pb.`comp_id` HAVING(COUNT(0) >= 10)
                )
                AND EXISTS (
